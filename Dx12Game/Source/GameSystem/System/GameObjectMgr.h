...
namespace GameObject
{
	...
	public:
		...
		/// <summary> 対象のTag内のアクティブなオブジェクト数を返却する </summary>
		static int GetActiveNum(const Tag _Tag);
		/// <summary> 名前が一致する有効中のゲームオブジェクトを取得 </summary>
		static Base* FindActiveObj(const std::string _GameObjName);
		/// <summary> 名前が一致する無効中のゲームオブジェクトを取得 </summary>
		static Base* FindDeactiveObj(const std::string _GameObjName);
		/// <summary> 全ての空間から名前が一致するゲームオブジェクトを取得 </summary>
		static Base* Find(const std::string _GameObjName);
		/// <summary> 全てのゲームオブジェクトを無効化 </summary>
		static void DeactivateAll();
		/// <summary> 名前が一致するゲームオブジェクトを全て無効化 </summary>
		static void DeactivateSomeAll(const std::string _GameObjName);
		/// <summary> ゲームオブジェクトをリストに追加、成功時true返却 </summary>
		static bool AddObject(Base* _Object);
	private:
		// Private Variable
