...
namespace GameObject
{
	...
	void Player::Update()
	{
		...
		if (this->state == State::Normal)
		{
			...
			this->cRestRect->Update();	// 移動処理後、フィールドから出てた場合、補正する
		...
		else if (this->state == State::Spawn)
		{
			...
			else if (magnification == 1)
			{
				...
				MyDX::Tool::Sound::PlaySE(Res::SEType::Generate);
			}
		}
	...
	void Player::Draw() const
	{
		if (this->isVisible)
		{
			...
			MyDX::Dx12Wrapper::DrawMesh(this->transform->matrix, Res::MeshType::Player, Res::MaterialType::Orange);
		...
		{
			// UI仮置き
			MyDX::Dx12Wrapper::DrawMesh2D(uiMatrix,Res::MeshType::Board,Res::MaterialType::U);
