...
namespace Input
{
	...
	void Mgr::UpdateKeyAxisL(const DirectX::Keyboard::State& _Key)
	{
		...
		if (_Key.A != _Key.D)
		{
			...
			if (_Key.D)
			{
				axisL.x += singleton->SpeedKeyAxis * Sys::Timer::GetDeltaTime();
				// 入力量が上限を超えていたら補正
				if (axisL.x > 1)
				{
					axisL.x = 1;
		...
		if (_Key.W != _Key.S)
		{
			...
			if (_Key.W)
			{
				axisL.y += singleton->SpeedKeyAxis * Sys::Timer::GetDeltaTime();
				// 入力量が上限を超えていたら補正
				if (axisL.y > 1)
				{
					axisL.y = 1;
	...
	void Mgr::UpdateKeyAxisR(const DirectX::Keyboard::State& _Key)
	{
		...
		if (_Key.Left != _Key.Right)
		{
			...
			if (_Key.Right)
			{
				axisR.x += singleton->SpeedKeyAxis * Sys::Timer::GetDeltaTime();
				// 入力量が上限を超えていたら補正
				if (axisR.x > 1)
				{
					axisR.x = 1;
		...
		if (_Key.Up != _Key.Down)
		{
			...
			if (_Key.Up)
			{
				axisR.y += singleton->SpeedKeyAxis * Sys::Timer::GetDeltaTime();
				// 入力量が上限を超えていたら補正
				if (axisR.y > 1)
				{
					axisR.y = 1;
