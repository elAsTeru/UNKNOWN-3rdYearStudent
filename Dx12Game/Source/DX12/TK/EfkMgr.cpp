...
namespace Effect
{
	...
	void EfkMgr::Init()
	{
		...
		// エフェクト読み込み
		typedef System::Helper::EnumIterator < Res::EfkType, Res::EfkType::Begin, Res::EfkType::End > typeIte;

		for (auto ite = typeIte(); ite != typeIte(Res::EfkType::End); ++ite)
		{
			const std::string tempId = std::to_string(ite.GetId());
			const std::wstring path = System::Helper::ToWstr(data[tempId.c_str()].GetString());

			for (auto ite2 = typeIte(); ite2 != typeIte(Res::EfkType::End); ++ite2)
			{
				const std::string tempId2 = std::to_string(ite2.GetId());
				const std::wstring path2 = System::Helper::ToWstr(data[tempId2.c_str()].GetString());
	...
	void EfkMgr::SyncronizeEffekseerCamera()
	{
		// Effekseer用ビュー行列
