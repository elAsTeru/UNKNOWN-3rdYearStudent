...
namespace MyDX::Tool
{
	...
	void Sound::LoadSE()
	{
		...
		// 2重ロードチェック後、データ読み込み
		typedef EnumIterator < Res::SEType, Res::SEType::Begin, Res::SEType::End > typeItr;
		for (auto itr = typeItr(); itr != typeItr(Res::SEType::End); ++itr)
		{
			...
			const std::wstring path = Sys::Helper::ToWstr(tempPath);
			// ファイル読み込み
	...
	void Sound::LoadBGM()
	{
		...
		// 2重ロードチェック後、データ読み込み
		typedef EnumIterator < Res::BGMType, Res::BGMType::Begin, Res::BGMType::End > typeItr;
