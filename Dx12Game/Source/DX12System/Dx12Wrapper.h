...
namespace MyDX
{
	constexpr uint16_t MaxMaterial = 100;
	...
	private:
		...
		// 旧定数バッファ←現在
		std::unordered_map<DRAW_TYPE, std::vector<ConstantBuffer*>>transform;	// 変換行列
		// 新定数バッファ
		//ConstantBuffer			meshCB[FrameCount * MAX_MESH_CB];	// メッシュ用バッファ
		//ConstantBuffer			transformCB[MAX_MESH_CB];			// 変換行列用バッファ


		Material					material[MaxMaterial];				//マテリアル


		std::unordered_map<DRAW_TYPE, ComPtr<ID3D12PipelineState>> pso;	// パイプラインステート
