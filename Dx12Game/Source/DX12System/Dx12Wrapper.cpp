...
namespace MyDX
{
    ...
    {
        {
            ...
            {
                if (!singleton->material[i].Init(
                    singleton->device.Get(),
                    singleton->pool[POOL_TYPE_RES],
            ...
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/normal",      singleton->material[0], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/white",       singleton->material[1], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/orange",      singleton->material[2], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/green",       singleton->material[3], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/unk",         singleton->material[4], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/default",     singleton->material[5], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/gray",        singleton->material[6], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/red",         singleton->material[7], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/u",           singleton->material[8], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/u2",          singleton->material[9], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/black",       singleton->material[10], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/purple",      singleton->material[11], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/yellow",      singleton->material[12], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/blue",        singleton->material[13], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/tenten",      singleton->material[14], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/gray2",       singleton->material[15], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/phase1",      singleton->material[16], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/phase2",      singleton->material[17], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/phase3",      singleton->material[18], batch);
            MySys::Res::MaterialMgr::SetTextureSet(L"Resource/Texture/phase4",      singleton->material[19], batch);
        ...
        // パイプラインステートの生成.
        {
            ...
            // 標準用GPS
            {
                ...
                desc.SampleDesc.Quality                             = 0;
                // GPS生成
                auto hr = singleton->device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(singleton->pso[DRAW_TYPE::UI].GetAddressOf()));
                CheckResult(hr);
                ...
                // GPS生成
                hr = singleton->device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(singleton->pso[DRAW_TYPE::TRIANGLE_BASIC].GetAddressOf()));
                CheckResult(hr);
            }
            ...
            {
                D3D12_GRAPHICS_PIPELINE_STATE_DESC desc{};
                desc.InputLayout = MyRes::LineVertex::InputLineLayout;
                desc.pRootSignature = singleton->BasicRS.Get();
                ...
                auto hr = singleton->device->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(singleton->pso[DRAW_TYPE::LINE].GetAddressOf()));
                if (FAILED(hr))
                {
        ...
        // 変換行列用の定数バッファの生成
        {
            ...
            // メモリ予約
            singleton->transform[DRAW_TYPE::TRIANGLE_BASIC].reserve(FrameCount* MaxBasicMeshCB);
            for (auto i = 0u; i < FrameCount * MaxBasicMeshCB; ++i)
            {
                ...
                singleton->transform[DRAW_TYPE::TRIANGLE_BASIC].push_back(pCB);
            }
            singleton->transform[DRAW_TYPE::TRIANGLE_BASIC].shrink_to_fit();

            singleton->transform[DRAW_TYPE::LINE].reserve(FrameCount* MaxLineRectCB);
            for (auto i = 0u; i < FrameCount * MaxLineRectCB; ++i)
            {
                ...
                singleton->transform[DRAW_TYPE::LINE].push_back(pCB);
            }
            singleton->transform[DRAW_TYPE::LINE].shrink_to_fit();


            singleton->transform[DRAW_TYPE::UI].reserve(FrameCount * MaxUICB);
            for (auto i = 0u; i < FrameCount * MaxUICB; ++i)
            {
                ...
                singleton->transform[DRAW_TYPE::UI].push_back(pCB);
            }
            singleton->transform[DRAW_TYPE::UI].shrink_to_fit();
        }
        return true;
    }
    ...
    void Dx12Wrapper::RenderBefore()
    {
        ...
        // 更新処理
        {
            ...
            {
                auto line = singleton->transform[DRAW_TYPE::LINE][singleton->frameIndex * 2 + i]->GetPtr<CbLine>();
            ...
            {
                auto mesh = singleton->transform[DRAW_TYPE::TRIANGLE_BASIC][singleton->frameIndex * 2 + i]->GetPtr<Transform>();
            ...
            {
                auto uiMesh = singleton->transform[DRAW_TYPE::UI][singleton->frameIndex * 2 + i]->GetPtr<Transform>();
    ...
    void Dx12Wrapper::Render()
    {
        ...
        // ここが描画処理
        {
            ...
            pCmd->SetPipelineState(singleton->pso[DRAW_TYPE::LINE].Get());
            // 線でできた四角を描画
            {
                pCmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_LINESTRIP); // インデックスを使用せずにライン描画
                for (int i = 0; i < singleton->drawRectData.size(); ++i)
                {
                    pCmd->SetGraphicsRootConstantBufferView(0, singleton->transform[DRAW_TYPE::LINE][singleton->frameIndex * 2 + i]->GetAddress());
            ...
            // メッシュ描画用に設定
            pCmd->SetPipelineState(singleton->pso[DRAW_TYPE::TRIANGLE_BASIC].Get());
            pCmd->SetGraphicsRootSignature(singleton->BasicRS.Get());
            pCmd->SetGraphicsRootConstantBufferView(3, singleton->dirLight->GetAddress());
            for (int i = 0; i < singleton->drawBasicMeshData.size(); ++i)
            {
                pCmd->SetGraphicsRootConstantBufferView(0, singleton->transform[DRAW_TYPE::TRIANGLE_BASIC][singleton->frameIndex * 2 + i]->GetAddress());
                pCmd->SetGraphicsRootDescriptorTable(1, singleton->material[singleton->drawBasicMeshData[i].matNum].GetTextureHandle(0, TU_BASE_COLOR));
                pCmd->SetGraphicsRootDescriptorTable(2, singleton->material[15].GetTextureHandle(0, TU_BASE_COLOR));
                MySys::Res::MeshMgr::GetMesh(singleton->drawBasicMeshData[i].mesh).Draw(pCmd);
            }
            // メッシュ描画リストのクリア
            singleton->drawBasicMeshData.clear();

            // UIメッシュ
            pCmd->SetPipelineState(singleton->pso[DRAW_TYPE::UI].Get());
            pCmd->SetGraphicsRootSignature(singleton->BasicRS.Get());
            for (int i = 0; i < singleton->draw2DUIData.size(); ++i)
            {
                pCmd->SetGraphicsRootConstantBufferView(0, singleton->transform[DRAW_TYPE::UI][singleton->frameIndex * 2 + i]->GetAddress());
                pCmd->SetGraphicsRootDescriptorTable(1, singleton->material[singleton->draw2DUIData[i].matNum].GetTextureHandle(0, TU_BASE_COLOR));
                pCmd->SetGraphicsRootDescriptorTable(2, singleton->material[10].GetTextureHandle(0, TU_BASE_COLOR));
                MySys::Res::MeshMgr::GetMesh(singleton->draw2DUIData[i].mesh).Draw(pCmd);
            }
    ...
    {
        // マテリアル破棄
        for (auto i = 0; i < MaxMaterial; ++i)
        {
            singleton->material[i].Term();
        }

        // 変換バッファ破棄
        for (size_t i = 0; i < singleton->transform[DRAW_TYPE::LINE].size(); ++i)
        {
            SafeTerm(singleton->transform[DRAW_TYPE::LINE][i]);
        }
        singleton->transform[DRAW_TYPE::LINE].clear();
        singleton->transform[DRAW_TYPE::LINE].shrink_to_fit();

        for (size_t i = 0; i < singleton->transform[DRAW_TYPE::UI].size(); ++i)
        {
            SafeTerm(singleton->transform[DRAW_TYPE::UI][i]);
        }
       singleton->transform[DRAW_TYPE::UI].clear();
       singleton->transform[DRAW_TYPE::UI].shrink_to_fit();
